...
import DestinationDecision from "../DestinationDecision";
...
/**
 * Phase 11 Step 4 — a mechanical extraction of Journey's original roadmap
 * rendering (previously the whole of app/(app)/timeline.tsx), moved here
 * unchanged so the Journey shell can switch between this and StoryView.
 * The ONE intentional behavioural change: the `if (loading)` early return
 * below now scopes only to this view's own content area rather than the
 * whole screen — the shell's ScreenHeader and Roadmap|Story segmented
 * control stay visible throughout, so Story remains reachable while this
 * loads. Everything else — useRoadmap, JourneyView, EstimateHeaderCard,
 * the Capability Gaps entry, every style — is unchanged from before.
 */
  ...
  return (
    <>
      ...
      {roadmap.alternativeCareers.length > 0 && (
        // Card itself only carries a bottom margin (its shared, app-wide
        // spacing rule) — nothing previously separated its top edge from
        // whatever came directly before it, so the destination content
        // above ran straight into this section with no visible gap. This
        // wrapper's marginTop is local to this one section, not a change to
        // Card's own spacing everywhere else it's used.
...
const styles = StyleSheet.create({
  ...
  alternativesSection: {
    // Same scale as the divider further below (pathwayDivider) uses to
    // separate the roadmap from an alternate pathway — a consistent,
    // deliberate beat between major sections rather than a one-off value.
